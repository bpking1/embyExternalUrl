...
最好不要自己猜,最直观的方式为同时打开 emby 媒体详情页查看最下方的入库全路径
和 alist 根目录查看地址栏去掉域名和端口的字符串,直接对比之后取差集即可
...
5.禁用指定客户端标识的 115 直链功能,走 emby 回源中转,
~~主要为部分客户端默认视频加载线程 > 2 超出了 115 服务方的限制,导致拖动进度条黑屏~~
...
1.[/skipImageCache]: 默认在 60 秒内跳过 nginx 图片缓存,到期自动关闭,注意此行为只影响 nginx,客户端/上游服务缓存属于外部系统,无法干预

1.1 [/skipImageCache:整数]: 整数 < 1 手动关闭跳过 nginx 图片缓存, 整数 > 0 设定跳过 nginx 缓存持续秒数,到期自动关闭

2.[/opendocs]: 默认在 60 秒内允许访问上游服务 docs,此参数到期自动失效,此时的值取决于 config 中的持久化配置

2.1 [/opendocs:整数]: 整数 < 1 手动失效此临时参数, 整数 > 0 设定此参数持续秒数

3.[/help]: 展示一些简单提示

4.[/showDictZoneState]: 展示 ningx 内存共享缓存各字典数目,即路由缓存/直链缓存

4.1 [/showDictZoneState[=字典名]]: 清空路由缓存/直链缓存,不加 =字典名 同时清空多级路由缓存,只允许指定路由缓存字典名
...
#### 36.IOS 几乎全部客户端播放 115 都会拖动进度条黑屏?

1.这些播放器之前可能会拖动进度条黑屏,但目前播放器那边貌似已经修复,建议尽快将脚本和播放器都升级到最新版本,
以下是一些当前版本号参考,并没有实际测试老版本情况, ios 不太方便安装老版本,
整了台 iPhone 8 Plus 测试,以下仅为测试 ios 16.7.10, 所有客户端均是最新版本,且为基础功能免费版,
且仅添加了`Emby`服务器,其余所有设置都是默认,额外需要确认脚本的`alist sign`参数是否真正对接成功

2.`HamHub: 1.3.9(267)`,`SenPlayer/4.2.1`,`VidHub/1.7.11`,`Fileball: 1.3.28(244)`,
`Infuse-Direct/8.0.2`,`Emby for iOS: 2.2.31`,
~~都是正常的,~~
部分已经正常,

3.测试媒体信息,`mp4 1080p H264 avc1 AAC yuvj420p`

4.假如还有拖动进度条黑屏,应该就只可能是播放器加载的媒体格式不支持,或 115 那边做了限制条件
